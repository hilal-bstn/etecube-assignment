...
export default class ProductService {
    async products(): Promise<any>
    {
        ...
        const result = await fetch('http://localhost:5000/product/products',
        {
    ...
    {
        const result = await fetch('http://localhost:5000/product/newproducts',
        {
            headers:{authorization: `bearer ${JSON.parse(localStorage.getItem('token')!)}`}
        });
   ...
   async productAdd (product:ProductModel) {
    ...
    async search(key:string) : Promise<any>
        {
            const result = await fetch(`http://localhost:5000/product/product-search/${key}`)
            const data = await result.json()
            return data;
        } 
};