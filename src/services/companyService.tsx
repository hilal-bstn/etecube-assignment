...
export default class CompanyService {
    async companies()
    {
        const result = await fetch('http://localhost:5000/company/companies',
        {
    ...
    async newCompanies()
    {
        const result = await fetch('http://localhost:5000/company/newcompanies',
        {
            headers:{authorization: `bearer ${JSON.parse(localStorage.getItem('token')!)}`}
        });
   ...
   async companyAdd (company:CompanyModel) {
        await fetch(`http://localhost:5000/company/add-company`,{
            method:'Post',
            body: JSON.stringify({company}),
            headers:{
            'Content-Type':"application/json",
    ...
    {
        const result = await fetch(`http://localhost:5000/company/company-search/${key}`)
        const data = await result.json()
        return data;
    } 
};